...
Функция ПолучитьСуммуФайлаSHA1(ПутьКФайлу) Экспорт
    ...
    Если ЭтоWindows Тогда
        Команда = СтрШаблон("certutil -hashfile %1", ПутьКФайлу);
	Иначе
...
//   <Тип.Вид>   - <описание возвращаемого значения>
//
Функция СравнитьФайлы(ПервыйФайПуть , ВторойФайлПуть) Экспорт
	Возврат ПолучитьСуммуФайлаSHA1(ПервыйФайПуть) = ПолучитьСуммуФайлаSHA1(ВторойФайлПуть);
КонецФункции